...
PassportConfigurator.prototype.setupModels = function (options) {
  ...
  this.roleMappingModel = options.roleMappingModel || loopback.getModelByType(this.app.models.RoleMapping);
  this.roleModel = options.roleModel || loopback.getModelByType(this.app.models.Role);
...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  function loginCallback(req, done) {
    return function (err, user, identity, token) {
      //todo: best to move this to after save of user identity
      if (authType === 'adfs') {
        if (identity && identity.profile && identity.profile.roles && identity.profile.roles.length) {
          for (var i = 0; i < identity.profile.roles.length; i++) {
            self.roleModel.findOne({where: {name: identity.profile.roles[i]}}, function (err, role) {
              if (err) return;
              else if (role) {
                role.principals({
                  where: {
                    principalId: (user.id || identity.profile.userId).toString(),
    ...
    case 'adfs':
      passport.use(name, new AuthStrategy(_.defaults({
        ...
        function (profile, done) {
          console.log(profile);
          if (profile) {
            var upn = profile['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/upn'],
            ...
            self.userIdentityModel.login(name, authScheme,
              userProfile, {identifier: upn}, options, loginCallback(null, done));
          }
        }
      ));
      break;
            ...
            req.logIn(user, function (err) {
              ...
              if (info && info.accessToken) {
                if (!!options.json) {
                  ...
                  try{
                    res.cookie('userName', (info.identity.profile.username || user.username), {
                      signed: req.signedCookies ? true : false,
                      maxAge: info.accessToken.ttl
                    });
                    res.cookie('userRoles', info.identity.profile.roles.join(";"), {
                      signed: req.signedCookies ? true : false,
                      maxAge: info.accessToken.ttl
                    });
                  }
                  catch(e){
                    console.log("passport-configurator: ",e);
                  }
                }
              }
